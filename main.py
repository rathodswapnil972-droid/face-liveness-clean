...
def detect_blink(gray):
    ...
    eyes = eye_detector.detectMultiScale(gray, 1.3, 5)
    visible = len(eyes) > 0
    if eye_visible_prev and not visible:
        blink_counter += 1
    eye_visible_prev = visible
    return blink_counter
...
async def analyze(file: UploadFile = File(...)):
    ...
    motion = detect_motion(frame)
    blink = detect_blink(gray)
    texture = detect_screen_texture(gray)
    label = predict_ai(frame)
    ...
    motion_avg = float(np.mean(motion_history))
    challenge_passed = (blink >= 1) or (motion_avg > 3.0)

    if texture > 1200:
        final = "FAKE"
    elif not challenge_passed:
        final = "FAKE"
    elif label != "live_video":
        final = "FAKE"
    else:
        final = "VERIFIED"
